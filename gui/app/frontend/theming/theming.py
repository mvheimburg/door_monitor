"""
This is an adaptation of KivyMD's theming behaviour to accomodate 
specific colours not in the palette.
...
from frontend.theming.colour_definitions import colors, palette, hue
...
class ThemeManager(Widget):
    ...
    def on_theme_style(self, instance, value):
        if (
            hasattr(MDApp.get_running_app(), "theme_cls")
            and MDApp.get_running_app().theme_cls == self
        ):
...
class ThemableBehavior():
    ...
    def __init__(self, **kwargs):
        if self.theme_cls is not None:
            pass
        elif hasattr(MDApp.get_running_app(), "theme_cls"):
